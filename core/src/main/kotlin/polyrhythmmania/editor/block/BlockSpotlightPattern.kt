...
class BlockSpotlightPattern(engine: Engine) : AbstBlockSpotlight(engine, BlockSpotlightPattern.BLOCK_TYPES) {
    ...
    init {
        this.width = 1f
        this.defaultText.bind { Localization.getVar("block.spotlightPattern.name").use() }
    }
...
//        events += compileRow(b, patternData.rowDpadTypes, world.rowDpad, EntityPiston.Type.PISTON_DPAD, 0, false, silent)
    ...
    override fun createContextMenu(editor: Editor): ContextMenu {
        return ContextMenu().also { ctxmenu ->
            ctxmenu.addMenuItem(LabelMenuItem.create(Localization.getValue("blockContextMenu.spotlightPattern"), editor.editorPane.palette.markup))
            ctxmenu.addMenuItem(SeparatorMenuItem())
            patternData.createMenuItems(editor, SpotlightActionType.NO_CHANGE, 0).forEach { ctxmenu.addMenuItem(it) }
        }
    }

    override fun copy(): BlockSpotlightPattern {
        return BlockSpotlightPattern(engine).also {
