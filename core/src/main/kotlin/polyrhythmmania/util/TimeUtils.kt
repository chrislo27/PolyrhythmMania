...
object TimeUtils {
    ...
    fun convertMsToTimestamp(ms: Float): String {
        ...
        return "${if (ms < 0) "${decimalFormatSymbols.minusSign}" else ""}${DecimalFormats.format("00", min)}:${DecimalFormats.format("00", sec)}${decimalFormatSymbols.decimalSeparator}${DecimalFormats.format("000", msPart)}"
    }
}