...
class PracticeTutorial2(main: PRManiaGame, val keyboardKeymap: InputKeymapKeyboard) : Practice(main) {
    ...
    init {
        ...
        practiceSection1.loopBlock = PracticeLoopBlock(4f) { engine, startBeat -> 
            ...
            listOf(0, 2, 4, 6).map { EngineInputter.RequiredInput(startBeat + it * 0.5f, InputType.A) }
        }
        ...
        practiceSection2.initBlock = PracticeInitBlock(8f, 3) { engine, startBeat ->
            ...
            engine.addEvent(EventRowBlockSpawn(engine, engine.world.rowDpad, 0, EntityPiston.Type.PLATFORM, startBeat + 4f + 3 * 0.5f))
            engine.addEvent(EventRowBlockSpawn(engine, engine.world.rowDpad, 1, EntityPiston.Type.PLATFORM, startBeat + 4f + 3 * 0.5f))
            engine.addEvent(EventRowBlockSpawn(engine, engine.world.rowDpad, 2, EntityPiston.Type.PLATFORM, startBeat + 4f + 3 * 0.5f))
            engine.addEvent(EventRowBlockSpawn(engine, engine.world.rowDpad, 3, EntityPiston.Type.PISTON_DPAD, startBeat + 4f + 3 * 0.5f))
            engine.addEvent(EventRowBlockSpawn(engine, engine.world.rowDpad, 6, EntityPiston.Type.PISTON_DPAD, startBeat + 4f + 6 * 0.5f))

            engine.addEvent(EventRowBlockSpawn(engine, engine.world.rowDpad, 9, EntityPiston.Type.PLATFORM, startBeat + 4f + 9 * 0.5f, affectThisIndexAndForward = true))
        }
        ...
        practiceSection3.loopBlock = PracticeLoopBlock(4f) { engine, startBeat ->
            ...
            engine.addEvent(EventPracticeRetract(engine, engine.world.rowDpad, 0, startBeat + 3.5f, affectThisIndexAndForward = true))
            listOf(
                    EngineInputter.RequiredInput(startBeat + 0 * 0.5f, InputType.A),
                    EngineInputter.RequiredInput(startBeat + 2 * 0.5f, InputType.A),
