...
import polyrhythmmania.PreferenceKeys.SETTINGS_SUBTITLE_OPACITY
...
class Settings(val main: PRManiaGame, val prefs: Preferences) { // Note: this predates PaintboxPreferences
    ...
    private val kv_windowedResolution: KeyValue<WindowSize> =
        KeyValue(SETTINGS_WINDOWED_RESOLUTION, PRMania.DEFAULT_SIZE)
    ...
    val kv_audioDeviceBufferCount: KeyValue<Int> =
        KeyValue(SETTINGS_AUDIODEVICE_BUFFER_COUNT, AudioDeviceSettings.getDefaultBufferCount())
    ...
    private val kv_calibrationDisableInputSFX: KeyValue<Boolean> =
        KeyValue(SETTINGS_CALIBRATION_DISABLE_INPUT_SFX, false)
    ...
    private val kv_forceTexturePack: KeyValue<ForceTexturePack> =
    ...
    val kv_editorPanningDuringPlayback: KeyValue<CameraPanningSetting> =
        KeyValue(EDITORSETTINGS_PANNING_DURING_PLAYBACK, CameraPanningSetting.PAN)
    ...
    private val kv_endlessDailyChallenge: KeyValue<DailyChallengeScore> =
        KeyValue(ENDLESS_DAILY_CHALLENGE, DailyChallengeScore.ZERO)
    ...
    val newIndicatorEditorHelpTexpack: NewIndicator =
        NewIndicator(NEW_INDICATOR_EDITORHELP_TEXPACK, Version(1, 1, 0), newEvenIfFirstPlay = false)
    val newIndicatorEditorHelpExporting: NewIndicator =
        NewIndicator(NEW_INDICATOR_EDITORHELP_EXPORTING, Version(1, 1, 0), newEvenIfFirstPlay = false)
    val newIndicatorEditorHelpPrmproj: NewIndicator =
        NewIndicator(NEW_INDICATOR_EDITORHELP_PRMPROJ, Version(1, 1, 0), newEvenIfFirstPlay = false)
    val newIndicatorEditorHelpSpotlights: NewIndicator =
        NewIndicator(NEW_INDICATOR_EDITORHELP_SPOTLIGHTS, Version(2, 0, 0), newEvenIfFirstPlay = false)
    val newIndicatorExtrasAssemble: NewIndicator =
        NewIndicator(NEW_INDICATOR_EXTRAS_ASM, Version(1, 1, 0), newEvenIfFirstPlay = false)
    val newIndicatorExtrasSolitaire: NewIndicator =
        NewIndicator(NEW_INDICATOR_EXTRAS_SOLITAIRE, Version(1, 2, 0), newEvenIfFirstPlay = false)
    val newIndicatorStoryMode: NewIndicator =
        NewIndicator(NEW_INDICATOR_STORY_MODE, Version(2, 0, 0), newEvenIfFirstPlay = true)
    val newIndicatorEndlessModeHelp: NewIndicator =
    ...
    fun load() {
        ...
        prefs.getEditorSetting(
            kv_editorPanningDuringPlayback as KeyValue<EditorSetting>,
    ...
    fun setStartupSettings(game: PRManiaGame) {
        PRManiaLocalePicker.attemptToSetNamedLocale(this.locale.getOrCompute())
    ...
    private fun Preferences.getEditorSetting(
        kv: KeyValue<EditorSetting>,
